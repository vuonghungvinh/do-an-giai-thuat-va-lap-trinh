...
void Nhap (LIST &L, Data* &room) {
	...
	do {
		...
		kiemtra=getch();
		if(kiemtra=='0')  break;
		cout<<endl<<"Ma hoa don:       ";
		fflush(stdin);
		gets(x.bill_no);
		cout<<endl<<"Ten khach hang:   ";
		fflush(stdin);
		gets(x.name);
		do {
			i=0;
		...
		do {
			...
			while(tmp1 !=NULL) {
				...
				if( (str1.compare(0,str2.length(),str2) ==0 ) ) {
					...
					//if(tmp->rm.status == 0){
					if(0) {
						cout<<endl<<"Phong dang ban vui long chon lai: " ;
						i=1;
						break;
					} else if(tmp1->rm.status == 2) {
						cout<<endl<<"Phong bi hu hong chon lai: " ;
						i=1 ;
						break;
...
void Search(LIST &L) {
	...
	int i=0;;
	p=L.head;
	cout<<endl<<"Nhap thong tin khach hang can tim: ";
	fflush(stdin);
	gets(tmp);
	cout<<endl<<"Ket qua tim kiem "<<endl;
	Header();
	while(p!=NULL) {
		if       (strcmp(p->data.name,tmp)==0) {
			output(p->data);    // ham strcmp dung de so sanh hai sau string xem giong nhau khong, neu giong tra ve 0
			cout<<endl;
			i++;
		} else if  (strcmp(p->data.bill_no,tmp)==0) {
			output(p->data);
			cout<<endl;
			i++;
			break ;
		} else if  (strcmp(p->data.identity_card,tmp)==0) {
			output(p->data);
			cout<<endl;
			i++;
			break;
		} else if  (strcmp(p->data.room_no,tmp)==0) {
			output(p->data);
			cout<<endl;
			i++;
			break;
...
void Remove(LIST &L) {
	...
	char t[19];
	p=L.head;
	cout<<endl<<"Nhap ten sinh vien can xoa: ";
	fflush(stdin);
	gets(t);
	while(p!=NULL) {
...
void Listed(LIST &L,int n) {
	...
	Header();
	while(p!=NULL) {
		if(n==1 && p->data.room_type=='A')     {
			output(p->data);
			cout<<endl;
			i++;
		} else if(n==2 && p->data.room_type=='B') {
			output(p->data);
			cout<<endl;
			i++;
		} else if(n==3 && p->data.room_type!= 'A' && p->data.room_type!= 'B') {
			output(p->data);
			cout<<endl;
			i++;
		}
